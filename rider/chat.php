...
if (!isset($_GET['order_id']) || !is_numeric($_GET['order_id'])) {
    ...
    echo "<h2>No deliveries available</h2>";
    echo "<a href='deliveries.php'>Go to Deliveries</a>";
    echo "</div>";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($message)) {
        ...
        mysqli_query($conn, "
            ...
            VALUES ($order_id, $rider_id, $customer_id, '$safe_msg')
        ");
    }
...
mysqli_query($conn, "
    UPDATE messages 
    SET is_read = 1 
    WHERE order_id = $order_id AND receiver_id = $rider_id
");

include '../includes/header.php';
?>

<div class="section-title">Chat with Customer</div>
...
<div style="max-width:900px;margin:auto;">
    ...
    ">
        ...
        <?php while ($msg = mysqli_fetch_assoc($messages_query)): ?>
            ...
            <div style="
                margin-bottom:10px;
                text-align:<?php echo ($msg['sender_id'] == $rider_id) ? 'right' : 'left'; ?>;
            ">
                ...
                <span style="
                    ...
                    border-radius:12px;
                    max-width:70%;
                    background:<?php echo ($msg['sender_id'] == $rider_id) ? '#10b981' : '#e5e7eb'; ?>;
                    color:<?php echo ($msg['sender_id'] == $rider_id) ? '#fff' : '#000'; ?>;
                ">
