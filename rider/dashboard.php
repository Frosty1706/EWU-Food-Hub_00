<?php
// File: C:\xampp\htdocs\EWU Food Hub\rider\dashboard.php
...
// Fetch delivery requests assigned to this rider with status 'assigned'
$delivery_requests = mysqli_query($conn, "SELECT o.*, c.full_name as customer_name, c.phone as customer_phone, r.restaurant_name
    FROM orders o
    JOIN users c ON o.customer_id = c.id
    JOIN restaurants r ON o.restaurant_id = r.id
    WHERE o.rider_id = $rider_id AND o.status = 'assigned'
    ORDER BY o.created_at DESC");

// Fetch deliveries in progress (Accepted, Picked, On the way)
$in_progress_statuses = "'accepted','picked','on_the_way'";
$deliveries_in_progress = mysqli_query($conn, "SELECT o.*, c.full_name as customer_name, c.phone as customer_phone, r.restaurant_name
    FROM orders o
    JOIN users c ON o.customer_id = c.id
    JOIN restaurants r ON o.restaurant_id = r.id
...
$completed_deliveries = mysqli_query($conn, "SELECT o.*, c.full_name as customer_name, r.restaurant_name
    FROM orders o
    JOIN users c ON o.customer_id = c.id
    JOIN restaurants r ON o.restaurant_id = r.id
...
<div class="section-title">Rider Dashboard</div>

<div class="stats-grid">
    <div class="stat-card">
        <div class="stat-icon teal">📦</div>
        <div class="stat-details">
            <h3><?php echo mysqli_num_rows($delivery_requests); ?></h3>
    ...
    <div class="stat-card">
        ...
        <div class="stat-details">
            <h3><?php echo mysqli_num_rows($deliveries_in_progress); ?></h3>
...
<div class="table-container">
    ...
    <?php if (mysqli_num_rows($delivery_requests) > 0): ?>
    <table>
        <thead>
            <tr>
                <th>Order ID</th>
        ...
        <tbody>
            <?php while ($order = mysqli_fetch_assoc($delivery_requests)): ?>
            <tr>
                ...
                <td>
                    <form method="POST" action="rider_process.php" style="display:inline-block;">
    ...
    <div class="empty-state">
        ...
        <p>You have no new delivery requests at the moment.</p>
    </div>
    <?php endif; ?>
</div>
...
<div class="table-container">
    ...
    <?php if (mysqli_num_rows($deliveries_in_progress) > 0): ?>
    <table>
        <thead>
            <tr>
                <th>Order ID</th>
        ...
        <tbody>
            <?php while ($order = mysqli_fetch_assoc($deliveries_in_progress)): ?>
            <tr>
                <td>#<?php echo $order['id']; ?></td>
                <td><?php echo htmlspecialchars($order['customer_name']); ?></td>
                <td><?php echo htmlspecialchars($order['customer_phone']); ?></td>
...
<div class="table-container">
    ...
    <?php if (mysqli_num_rows($completed_deliveries) > 0): ?>
    <table>
        <thead>
            <tr>
                <th>Order ID</th>
        ...
        <tbody>
            <?php while ($order = mysqli_fetch_assoc($completed_deliveries)): ?>
            <tr>
                <td>#<?php echo $order['id']; ?></td>
                <td><?php echo htmlspecialchars($order['customer_name']); ?></td>
                <td><?php echo htmlspecialchars($order['restaurant_name']); ?></td>
